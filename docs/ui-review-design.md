...
- Annotation lists use the shared library/annotation rail described above. Metadata retains quick editing; drafts survive closing and switching papers in host-owned storage. Catalog titles open the reader directly (metadata-only records show their no-PDF surface without fetching pages); row action buttons keep edit/select semantics.
