...
The continuous-annotation-reference update has passed **151 JavaScript tests, 48 Python tests, 13 actual Harness integration checks and 11 browser flows**. The updated existing profile, authenticated capability, served assets and refreshed native library pane have also been verified. A new isolated memory run covers the synthetic 2,000-record/1,000-PDF fixture; a separate browser walkthrough records four Host/browser samples. Earlier evidence is retained with its original scope. User literature was not migrated, and no paid model was used. Original code and dependency licenses are distinguished in `THIRD_PARTY.md`.

## Native conversation checks

The current **151 JavaScript / 48 Python** run is recorded in the [automated receipt](validation/automated.json), including client build, browser syntax and CSL integrity. It covers exact source extraction beyond 40 notes and the old per-note preview length, changed/deleted sources, external duplicate annotation IDs, explicit coverage/budget failure, private immutable snapshots and retry bindings. Client cases verify additive selection, filtered pagination, later-note suggestions, question/selection recovery, native scoped chips and persisted plain-token inspection. Host cases verify actual source/body hashes, durable flush failure, usage retained after later history windows, wrong-paper/session references, multiple-group budgets and Unicode code-point counting. The earlier cold Session, model inheritance, PDF feedback and acquisition cases continue to pass. Reproduce with `node scripts/validate.mjs`, or `npm test` and `uv run --offline pytest -q`.
...
The 26/60-test intake receipt is preserved in [Git history at the intake milestone](https://github.com/mappedinfo/dsh-paper-library/blob/8d7b4a5/docs/validation/automated.json). The 101/39 conversation run is historical; current `automated.json` records 151/48. Other historical checks include [earlier installed profile](validation/install.json), [preserved prior configuration](validation/install-preservation.json) and [earlier manual walkthrough](validation/manual.json).
