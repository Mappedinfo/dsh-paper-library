...
The language and host-persistence update passes **278 JavaScript tests, 105 Python tests, 12 language browser flows and 6 actual Harness language checks**. The 17 reading, 13 workbench and 11 native-reference browser flows were rerun. The installed host was restarted after an authenticated idle check; language, durable state and retained capabilities/assets pass [installation verification](validation/language-install.json). The standalone review page was visibly refreshed and its language panel opened. No external model was used. Previous 2,000-record/1,000-PDF memory receipts retain their original scope; they are not new language memory benchmarks. Original code and dependency licenses remain distinguished in `THIRD_PARTY.md`.
...
The current **278 JavaScript / 105 Python** run is recorded in the [automated receipt](validation/automated.json), including client build, browser syntax and CSL integrity. New cases cover private atomic state writes, path safety, concurrent CAS, request/record UTF-8 bounds, legacy migration, asynchronous draft restoration, current-paper routing, generation retries and source-grounded vocabulary. Reading cases include rotated annotations, geometry-only reading, page eviction, hidden chat polling and panel/selection restoration. Existing catalog/graph, exact source extraction, source budgets, immutable snapshots, native logged usage, additive selection, cold Sessions, model inheritance and acquisition cases continue to pass. Reproduce with `node scripts/validate.mjs`, or `npm test` and `uv run --offline pytest -q`.
...
The 26/60-test intake receipt is preserved in [Git history at the intake milestone](https://github.com/mappedinfo/dsh-paper-library/blob/8d7b4a5/docs/validation/automated.json). The 101/39 conversation, 151/48 reference, 184/82 workbench and 222/105 reader runs are historical. Other historical checks include [earlier installed profile](validation/install.json), [preserved prior configuration](validation/install-preservation.json) and [earlier manual walkthrough](validation/manual.json).
