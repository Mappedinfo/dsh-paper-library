...
The reading workspace passes **222 JavaScript tests, 105 Python tests, 17 reading browser flows, 13 workbench browser flows and 11 native-reference browser flows**. The installed host was restarted after an authenticated idle check; capabilities and assets pass [installation verification](validation/reader-install.json). The standalone review page was visibly refreshed. No external model was used. The previous reference milestone's native Host and 2,000-record/1,000-PDF memory receipts retain their original scope; they are not new reader memory benchmarks. Original code and dependency licenses remain distinguished in `THIRD_PARTY.md`.
...
The current **222 JavaScript / 105 Python** run is recorded in the [automated receipt](validation/automated.json), including client build, browser syntax and CSL integrity. New cases cover native annotation types on rotated pages, geometry-only reading, page eviction and stale responses, bounded metadata drafts, selected-paper routing, hidden chat polling and panel/selection restoration. Existing catalog/graph, exact source extraction, source budgets, immutable snapshots, native logged usage, additive selection, cold Sessions, model inheritance and acquisition cases continue to pass. Reproduce with `node scripts/validate.mjs`, or `npm test` and `uv run --offline pytest -q`.
