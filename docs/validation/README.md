...
| `board-harness.json` | `node scripts/board-harness-smoke.mjs`（native host） | 2026-09-18 | Isolated native DSH profile with a deterministic keyless model and a synthetic library; authenticated whiteboard records, immutable snapshot, revision-checked writes, too |
