...
export const TOOL_SPECS = [
  ...
  { name: 'library_challenges', action: 'challenge_scan', title: 'Mine research difficulty candidates', parameters: { ids: { type: 'array', items: { type: 'string' }, required: true, description: 'Explicit paper ids (1–50); the scan never searches the library on its own' }, sections: { type: 'array', items: { type: 'string' }, description: 'Optional subset of known section names' }, persist: { type: 'boolean', description: 'Freeze one paper\'s candidates as immutable reviewable sources; requires exactly one id' } } },
  { name: 'library_challenge_themes', action: 'challenge_themes', title: 'Aggregate saved difficulty records into reviewable corpus themes', mutate: true, parameters: { ids: { type: 'array', items: { type: 'string' }, required: true, description: 'Explicit paper ids (1–200) whose saved difficulty drafts are aggregated; drafts are read, no PDF is opened' }, include: choice(['accepted', 'all'], 'accepted (default) uses reviewed drafts only; all also includes needs-review drafts'), persist: { type: 'boolean', description: 'Store the themes as needs-review records; default true' } } },
  { name: 'library_challenge_theme_review', action: 'challenge_theme_review', title: 'Accept or reject a challenge theme', mutate: true, parameters: { id: string('Theme id (ct-…)', true), decision: choice(['accepted', 'rejected'], 'Reviewer decision', true), reviewed_by: { type: 'string', enum: ['user'], required: true, description: 'Only an explicit human review may decide' }, expected_revision: { type: 'integer', required: true, description: 'Current theme revision; a mismatch is a conflict, not a retry' } } },
  { name: 'library_challenge_theme_merge', action: 'challenge_theme_merge', title: 'Merge selected challenge themes after review', mutate: true, parameters: { theme_ids: { type: 'array', items: { type: 'string' }, required: true, description: '2–10 theme ids from one corpus scope' }, expected_revisions: { type: 'array', items: { type: 'integer' }, required: true, description: 'Revisions aligned positionally with theme_ids; a mismatch is a conflict, not a retry' }, label: string('Optional merged theme label; omitted keeps the representative label'), reviewed_by: { type: 'string', enum: ['user'], required: true } } },
  { name: 'library_challenge_export', action: 'challenge_export', title: 'Export reviewed challenge themes as CSV, Markdown and BibTeX', mutate: true, parameters: { ids: { type: 'array', items: { type: 'string' }, required: true, description: 'The corpus paper ids that define the export scope' }, scope: string('Corpus scope hash returned by a previous aggregation'), include_unreviewed: { type: 'boolean', description: 'Also export needs-review themes; default false' } } },
  { name: 'library_challenge_theme_check', action: 'challenge_theme_check', title: 'Structurally check stored challenge themes', parameters: { scope: string('Corpus scope hash returned by an aggregation', true), statuses: { type: 'array', items: { type: 'string' }, description: 'Optional subset of needs-review/accepted/rejected/merged' } } },
  { name: 'library_challenge_comparison', action: 'challenge_comparison', title: 'Compare themes against a user-owned checklist', mutate: true, parameters: { ids: { type: 'array', items: { type: 'string' }, required: true, description: 'The corpus paper ids that define the scope' }, scope: string('Corpus scope hash returned by an aggregation'), checklist_text: string('Pasted checklist text (at most 65536 characters)'), checklist_path: string('Local text/Markdown/JSON/CSV/BibTeX file (at most 1 MiB)'), checklist_label: string('How this checklist should be cited in the report'), checklist_date: string('Known YYYY-MM-DD; a file path otherwise records its modified date') } },
  { name: 'library_challenge_review_packet', action: 'challenge_review_packet', title: 'Write the human review packet for stored themes', mutate: true, parameters: { ids: { type: 'array', items: { type: 'string' }, required: true, description: 'The corpus paper ids that define the scope' }, scope: string('Corpus scope hash returned by an aggregation'), comparison_id: string('Optional stored comparison to include'), statuses: { type: 'array', items: { type: 'string' }, description: 'Optional subset of needs-review/accepted/rejected/merged' } } },
  ...
  { name: 'library_bibliography', action: 'bibliography_build', title: 'Build canonical references.bib with identity audit', mutate: true, parameters: { verify: { type: 'boolean', description: 'Explicitly verify DOIs against public registration; read-only comparison, default false' }, verify_limit: { type: 'integer', description: '1–100 DOIs to verify this run; default 25' }, include_datasets: { type: 'boolean', description: 'Include datasets and releases; default true' } } },
