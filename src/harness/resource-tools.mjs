...
export const RESOURCE_TOOL_SPECS = [
  ...
  {name:'library_knowledge',action:'knowledge_tool',title:'Save selected evidence and propose reviewable knowledge',mutate:true,parameters:{operation:operations(['source_put','source_get','source_check','source_list','draft_put','draft_get','draft_list','draft_lint','note_get','note_list','export']),input_json:text('JSON object. Entity is {kind:paper|dataset|release,id}. source_put takes entity,kind,text or annotation_ref; draft_put takes entity,source_ids,mode and note title/body or typed nodes/edges/assertions; draft_lint takes {id} and reports review findings without changing anything. All model proposals await user review; this tool cannot accept drafts. Reads/exports require explicit IDs or entity scope.',true)}},
]
