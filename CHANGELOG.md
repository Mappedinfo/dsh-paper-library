...
- **只建链接、不复制**：扫描只读目录项与 size/mtime，不打开 PDF、不算哈希，未变化文件直接跳过；未写过的论文不产生副本。实测 425 个 PDF / 约 4.6 GB：首次扫描 0.2 秒量级、增量重扫 0.1 秒量级、目录约 2 MB。每次调用默认最多索引 200 个新文件（上限 2000）并返回剩余量，几千篇文献分批完成。
...
- 验证：**527 JavaScript / 234 Python** 测试，含 `tests-js/external-sources.test.mjs`（配置校验、同步配置读取与缓存、设置合并）与 `tests/test_external.py`（索引、增量、改名、缺失恢复、复制写入、越界拒绝）。
