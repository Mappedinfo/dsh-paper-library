...
语言学习与本机保存通过两套独立浏览器上下文的 12 项流程，以及真实 DSH 的 6 项集成检查：覆盖翻译、优化、难词积累、跨浏览器草稿、冲突恢复和重启后的幂等重试。阅读、目录工作台、主对话引用分别通过 17、13、11 项浏览器回归。模型输出采用确定性本地替身，没有外部模型请求；这验证功能与存储，不代表真实模型质量。自动化测试、安装核验和此前容量测量的准确范围见[验证记录](docs/validation.md)。
