...
2. 搜索标题、作者、年份、DOI、引用键、标签或摘要。侧栏卡片用于选文阅读；「文献库」或「展开表格」用于排序、分页和维护资料。表格选中一行只读取元数据，进入阅读后才打开 PDF。PDF 连续滚动，只保留当前页附近最多三页的图像和文字；页码输入框可直接跳转。「全屏阅读」扩展阅读区域，Esc 返回。阅读工具栏提供缩放：默认「全宽」自适应窗宽，可用 −／＋、直接输入 25–400 的百分比，或点「全宽」复位；放大后页面横向滚动，放大会按需提高渲染密度以免文字发虚。阅读与批注栏之间有一条分隔条，可拖动无级调整侧栏宽度（160–720 px，键盘方向键也可微调），PDF 随窗宽自动重新适配；宽度与缩放保存在主机，换浏览器或刷新后恢复。
