...
4. 顶部工具栏按当前操作切换。「引用与导出」提供 APA 7、BibLaTeX、批注 PDF 和批注文本导出；「资料」打开快速编辑侧栏。资料草稿保存在运行 DSH 的本机，关闭页面、切换论文或换浏览器后可恢复。APA 7 同时提供富文本和纯文本；「导出文献库」导出全库。
...
当前主题更新通过 289 项 JavaScript、105 项 Python 测试，以及 12 项主题浏览器检查。检查覆盖浅色／深色、430／741／1400 px 布局、键盘焦点、抽样文字对比度和不刷新页面的主题／字号同步。同步检查使用真实 DSH 样式和插件桥接代码，主题事件来自隔离测试环境；本机重启后的认证服务另行通过资源核验。此次检查没有调用模型。

此前语言学习与本机保存通过两套独立浏览器上下文的 12 项流程，以及真实 DSH 的 6 项集成检查：覆盖翻译、优化、难词积累、跨浏览器草稿、冲突恢复和重启后的幂等重试。该里程碑也验证了阅读、目录工作台、主对话引用的 17、13、11 项浏览器流程；这些专项流程未在主题更新中重跑。模型输出采用确定性本地替身，没有外部模型请求。自动化测试、安装核验和此前容量测量的准确范围见[验证记录](docs/validation.md)。
