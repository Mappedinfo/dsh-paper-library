...
2. 搜索标题、作者、年份、DOI、引用键、标签或摘要。侧栏卡片用于选文阅读；「展开表格」用于排序、分页和维护资料。表格中选中一行只读取元数据，点击「阅读」才打开 PDF；一次只显示一页。
3. 选中文字后点击「高亮并批注」，或添加页批注。保存完成后批注已写进管理副本 PDF。
4. 顶部工具栏跟随当前文献，提供「复制 APA 7」「导出 BibLaTeX」、批注 PDF 下载、资料编辑、附件与翻页操作。APA 7 同时提供富文本和纯文本；「导出文献库」导出全库。
