...
6. 打开论文时，插件自动建立或复用这篇论文的 **DSH 对话**。通过「新增与更新」「选择…」或「全部」建立本次引用集合，可混选新旧批注、增删条目，并附加临时选文。之后保存的批注会提示加入，不会改变已有选择。插件内与 DSH 主页面使用同一段历史。
...
当前批注引用更新已通过 151 项 JavaScript、48 项 Python 测试，以及隔离 Harness 的 13 项原生检查。浏览器的 11 项交互检查也通过，覆盖新旧混选、连续新增、编辑后核对、46 条全选、原生标签、刷新恢复、主对话发送和窄面板；展开检查器后主列与 PDF 面板没有横向溢出。原生集成检查生成两次、本轮浏览器生成三次确定性本地回复，没有外部模型请求。新版已通过官方 CLI 更新到现有配置，重启后确认批注引用能力生效，并刷新、重新打开了本地 DSH 文献库。具体范围见[验证记录](docs/validation.md)。
