...
const realtimeCompanion = status.result.realtime_companion === true
if (flags.get('--require-companion') === 'true') {
  assert.equal(realtimeCompanion,true,'Restarted host lacks the durable companion queue')
  for(const file of ['companion.js','app.js','paper-chat.js','index.html']) {
    const asset=await fetch(`${address.origin}/api/paper-library/${file}`,{headers,signal:AbortSignal.timeout(10000)})
    assert.equal(asset.status,200,`Missing companion asset: ${file}`)
    assert.equal(await asset.text(),await readFile(new URL(`../web/${file}`,import.meta.url),'utf8'),`Installed companion asset differs from source: ${file}`)
  }
}
...
if (flags.get('--require-analysis') === 'true') {
  ...
  for(const [file,marker] of [['paper-analysis.js','PaperAnalysis'],['paper-analysis.css','paper-analysis-panel'],['theme.css','.pane-heading .search-field']]) {
    const asset=await fetch(`${address.origin}/api/paper-library/${file}`,{headers,signal:AbortSignal.timeout(10000)})
    assert.equal(asset.status,200,`Missing selected-paper analysis asset: ${file}`)
    assert.ok((await asset.text()).includes(marker),`Stale selected-paper analysis asset: ${file}`)
  }
}
