...
try{
  ...
  for(const mode of ['light','dark']){
    ...
    for(const width of [430,741,1400]){
      ...
      await page.locator('[data-tab="annotations"]').click();await page.locator('.library-pane #annotations-tab').waitFor();assert.equal(await page.locator('#reading-side-panel').isVisible(),false);await page.locator('#annotation-list .annotation-card').first().waitFor();await contrast(page,['#annotation-list .annotation-card','#reading-sidebar-side'],`${mode} annotations ${width}`);await noOverflow(page,`${mode} annotations ${width}`);await shot(page,`${mode}-annotations-${width}`);await page.locator('#reading-sidebar-library').click();
