/** Explicit local setup: enable analysis defaults, or only live companion via --companion true.
 * Uses the authenticated native settings API and its revision fence. Credentials
 * remain in memory; the optional receipt contains only the requested choices.
 */
...
assert.ok(log&&Number.isInteger(port)&&port>0&&port<65536,'Usage: --log PRIVATE_HOST_LOG --port PORT [--companion true] [--output RECEIPT]')
...
const patch=flags.get('--companion')==='true'?{'auto-paper-conversation':true}:{auto_analysis:true,analysis_fill:true}
...
const report={verified_at:new Date().toISOString(),ok:true,backend:'dsh',changed,...patch,otherPreferencesPreserved:true,privateDocumentsRead:false,modelRequestsMadeByScript:0}
