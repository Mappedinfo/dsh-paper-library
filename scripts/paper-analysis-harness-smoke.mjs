...
try {
  ...
  const observed=await host.observe(session.sessionId); assert.equal(observed.generations,1); assert.equal(observed.analysis.length,1)
  ...
  assert.equal(observed.analysisAgents.length,2); assert.equal(observed.analysisAgents.every(agent=>agent.origin==='subagent'&&!agent.loaded),true)
  assert.equal(observed.analysisAgents[1].parent,observed.analysisAgents[0].id)
  assert.deepEqual(observed.analysisGuard,{attempts:1,denied:1,executed:0})
  ...
  assert.deepEqual((await host.observe(session.sessionId)).observations,[{sessionLoaded:false,agentLoaded:false}])
  assert.equal((await host.observe(session.sessionId)).generations,1)
  ...
  assert.deepEqual((await host.observe(session.sessionId)).analysisGuard,{attempts:2,denied:2,executed:0})
