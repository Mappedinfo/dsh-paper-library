...
try {
  ...
  const observed=await host.observe(session.sessionId); assert.equal(observed.generations,3); assert.equal(observed.analysis.length,1)
  assert.equal(observed.analysisNotes.length,1); assert.equal(observed.analysisNotes[0].batches,1)
  assert.equal(observed.analysisReviews.length,1); assert.equal(observed.analysisReviews[0].batches,1)
  ...
  assert.equal(observed.analysisAgents.length,6,'One isolated agent per generation: batch, reading note and evidence-atlas review');
  assert.equal(observed.analysisAgents.every(agent=>agent.origin==='subagent'&&!agent.loaded),true)
  ...
  assert.equal(observed.analysisAgents[5].parent,observed.analysisAgents[4].id,'The review generation owns its own isolated agent')
  assert.deepEqual(observed.analysisGuard,{attempts:3,denied:3,executed:0},'Every generation agent stays denied scoped tools')
  ...
  assert.deepEqual((await host.observe(session.sessionId)).observations,[{sessionLoaded:false,agentLoaded:false}])
  assert.equal((await host.observe(session.sessionId)).generations,3)
  ...
  assert.deepEqual((await host.observe(session.sessionId)).analysisGuard,{attempts:4,denied:4,executed:0})
  ...
  // The automatic evidence-atlas review is a separate, bounded draft.
  assert.ok(full.review_draft_id,'the same run produced an evidence-atlas review draft')
  assert.deepEqual({...full.review_coverage,sources:'bounded'},{batches:2,batches_total:2,partial:false,sources:'bounded'})
  const review=await host.api({action:'knowledge_draft_get',id:full.review_draft_id})
  assert.equal(review.status,'needs-review');assert.equal(review.mode,'note');assert.equal(review.origin,'llm')
  assert.match(review.title,/^证据图谱评审/);assert.notEqual(review.id,full.note_draft_id)
  for(const section of ['快速判定','案例拆解','状态矩阵','归因','联合覆盖','形容词','本文最多能声称','本文不能声称','修改清单']) assert.match(review.body,new RegExp(section))
  assert.equal(full.review_profile,null,'No private overlay is configured in this isolated host')
  record('the-automatic-review-saves-a-pending-evidence-atlas-draft-with-every-required-section')
  ...
  assert.equal(fullObserved.generations,4,'Two batches, one reading note and one evidence-atlas review');assert.equal(fullObserved.analysis.every(batch=>batch.sourceIds.length<=8),true)
  assert.equal(fullObserved.analysisNotes.length,1);assert.equal(fullObserved.analysisNotes[0].batches,2)
  assert.equal(fullObserved.analysisReviews.length,1);assert.equal(fullObserved.analysisReviews[0].batches,2);assert.equal(fullObserved.analysisReviews[0].overlay,false)
  ...
  report={verified_at:new Date().toISOString(),ok:true,checks,deterministicModelGenerations:9,completedGenerations:7,readingNoteGenerations:2,evidenceAtlasReviews:2,cancelledOrInterruptedGenerations:2,replayGenerationsAfterHostRestart:0,nativeAgentsPerRun:2,scopedToolProbeExecutions:0,externalModelRequestsMade:0,sourceData:'Five-page scoped fixture and thirteen-page automatically queued full-text fixture',limitations:['Native spawn routing, lifecycle and durable jobs only; no real provider output-quality or real-library capacity claim']}
