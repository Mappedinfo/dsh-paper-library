...
try {
  ...
  await page.locator('#search').fill(paper.title);await page.locator(`.paper-card[data-id="${paper.id}"]`).waitFor();await page.locator(`.paper-card[data-id="${paper.id}"]`).click();await page.locator('#pdf-page').waitFor();
  ...
  if(process.argv.includes('--recon')){console.log(JSON.stringify({run:relative(project,run),dom:await page.locator('body').innerText(),errors}));}
    ...
    await page.locator('#catalog-create').click();await page.locator('#edit-title').fill('UI synthetic newly created paper');await page.locator('#edit-authors').fill('Example, Taylor');await page.locator('#edit-year').fill('2026');await page.locator('#edit-journal').fill('Fixture Journal');await page.locator('#edit-affiliations').fill('1 | Fixture Institute');await page.locator('#edit-received').fill('2025-10-12');
    ...
    await page.locator('#search').fill(paper.title);await page.locator('#catalog-table .table-title').filter({hasText:paper.title}).waitFor();await page.locator('#catalog-table tbody tr').first().getByRole('button',{name:'阅读',exact:true}).click();await page.locator('#pdf-page').waitFor();
    await page.locator('#metadata-open').click();await page.locator('#edit-title').fill(paper.title+' revised');await page.locator('#metadata-form button[type="submit"]').click();await page.locator('#metadata-dialog').waitFor({state:'hidden'});
    ...
    for(const [type,label] of [['method','Matched pair comparison'],['dataset','Synthetic observations'],['claim','A synthetic testable claim'],['evidence','Synthetic supporting observation']]){
      await page.locator('[data-kg="add-node"]').click();const form=page.locator('[data-kg="form"]');await form.locator('[name="type"]').selectOption(type);await form.locator('[name="label"]').fill(label);await form.locator('[name="page"]').fill('2');await form.locator('[name="quote"]').fill('Synthetic exact passage for fixture validation.');await form.locator('[name="note"]').fill('Reader assertion for a synthetic test.');await page.locator('[data-kg="save"]').click();await page.locator('[data-kg="dialog"]').waitFor({state:'hidden'});
    }
    ...
    await page.getByRole('button',{name:/论据 · Synthetic supporting observation/}).first().click();await page.locator('[data-kg="inspector"]').getByRole('button',{name:'返回 PDF 第 2 页',exact:true}).click();await page.waitForFunction(()=>document.getElementById('page-number').value==='2');await page.locator('#pdf-page').waitFor();record('graph-evidence-returns-to-known-pdf-page');
    await page.setViewportSize({width:430,height:800});assert.equal(await overflow(),false);await page.screenshot({path:join(run,'reader-430.png')});await page.setViewportSize({width:1400,height:950});assert.equal(await overflow(),false);await page.screenshot({path:join(run,'reader-1400.png')});record('430-and-1400px-reading-layout-has-no-document-overflow');
    await page.reload();await page.waitForLoadState('networkidle');await page.locator('#search').fill(paper.title);await page.locator(`.paper-card[data-id="${paper.id}"]`).waitFor();await page.locator(`.paper-card[data-id="${paper.id}"]`).click();await page.locator('#pdf-page').waitFor();await page.locator('[data-tab="graph"]').click();await page.locator('[data-kg="canvas"] svg').waitFor();assert.match(await page.locator('[data-kg="relations"]').innerText(),/支持/);record('fresh-page-reloads-saved-graph-and-rich-metadata');
    ...
    await page.reload();await page.waitForLoadState('networkidle');await page.locator('#search').fill(paper.title);await page.locator(`.paper-card[data-id="${paper.id}"]`).click();await page.locator('#pdf-page').waitFor();await page.locator('[data-tab="graph"]').click();await page.getByRole('button',{name:'方法 · Revised matched pair method',exact:true}).waitFor();assert.equal(await page.getByRole('button',{name:'数据 · Synthetic observations',exact:true}).count(),0);assert.equal(await page.locator('[data-kg="relations"] .kg-relation-name').filter({hasText:'矛盾'}).count(),0);record('visible-node-delete-and-fresh-reload-retain-edits-and-deletions-with-pdf-intact');
