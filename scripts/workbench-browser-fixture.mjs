...
try {
  ...
  if(process.argv.includes('--recon')){console.log(JSON.stringify({run:relative(project,run),dom:await page.locator('body').innerText(),errors}));}
    ...
    await page.waitForFunction(()=>{const number=document.getElementById('page-number').value;const image=document.querySelector(`.pdr-sheet[data-pdf-page="${number}"] img`);return image?.complete&&image.naturalWidth>=image.clientWidth*.95;});
    ...
    await page.reload();await page.waitForLoadState('networkidle');await page.locator('#search').fill(paper.title);await page.locator(`.paper-card[data-id="${paper.id}"]`).waitFor();await page.locator(`.paper-card[data-id="${paper.id}"]`).click();await page.locator('.pdr-sheet[data-loaded="true"]').first().waitFor();await page.locator('[data-tab="graph"]').click();await page.locator('[data-kg="canvas"] svg').waitFor();assert.match(await page.locator('[data-kg="relations"]').innerText(),/支持/);record('fresh-page-reloads-saved-graph-and-rich-metadata');
    ...
    await page.reload();await page.waitForLoadState('networkidle');await page.locator('#search').fill(paper.title);await page.locator(`.paper-card[data-id="${paper.id}"]`).click();await page.locator('.pdr-sheet[data-loaded="true"]').first().waitFor();await page.locator('[data-tab="graph"]').click();await page.getByRole('button',{name:'方法 · Revised matched pair method',exact:true}).waitFor();assert.equal(await page.getByRole('button',{name:'数据 · Synthetic observations',exact:true}).count(),0);assert.equal(await page.locator('[data-kg="relations"] .kg-relation-name').filter({hasText:'矛盾'}).count(),0);record('visible-node-delete-and-fresh-reload-retain-edits-and-deletions-with-pdf-intact');
