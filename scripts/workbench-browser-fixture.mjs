...
try {
  ...
  if(process.argv.includes('--recon')){console.log(JSON.stringify({run:relative(project,run),dom:await page.locator('body').innerText(),errors}));}
    ...
    await page.locator('#catalog-table .table-title').first().click();
    await page.locator('#no-pdf').waitFor({state:'visible'});
    assert.equal(await page.locator('.workspace').evaluate(node=>node.classList.contains('catalog-expanded')),false,'Title click leaves the table for reading');
    assert.equal(await page.locator('#no-pdf').isVisible(),true,'A metadata-only record opens its no-PDF surface');
    assert.equal(apiCalls.filter(c=>c.action==='page').length,pagesBefore,'Opening a metadata-only record never fetches PDF pages');
    assert.equal(await overflow(),false);
    await page.locator('#workspace-library').click();await page.locator('#catalog-table tbody tr').first().waitFor();
    record('server-sort-search-40-row-pagination-and-title-click-opens-reading');
    ...
    await page.locator('#search').fill('UI synthetic');
    await page.locator('#catalog-table .table-title').filter({hasText:'newly created'}).waitFor({timeout:60000});
    ...
    await page.locator('#search').fill(paper.title);await page.locator('#catalog-table .table-title').filter({hasText:paper.title}).waitFor();await page.locator('#catalog-table .table-title').first().click();await page.locator('.pdr-sheet[data-loaded="true"]').first().waitFor();
